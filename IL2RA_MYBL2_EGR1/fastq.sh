...
experiment_name=$1
experiment_type=$2 #Use ME if MYBL2+EGR1 insertions, else use "EGR" or "MYB"
...
insertion_sequences="${experiment_name}/insertion_sequences.csv" # insertion names and sequences
...
python make_needleall_bash_MYBL2.py "$fastq_path_info" "$alignment_files_folder" "$experiment_name" "$experiment_type"
bash run_needle.sh
...
python raw_counts.py "$min_alignment_score" "$genomic_context_file" "$insertion_sequences" "$experiment_name" "$experiment_type"

# score all variants to decide filtering threshold
python counts_to_scores.py "$experiment_name" "$experiment_type"
