...
for gene in genes:
    gene_predictions={}
    data=pd.read_csv(f"{data_directory}jurkat/6_mer_{gene}_PETRA_and_predictions_jurkat.csv")
    ...
    jurkat_predictions[gene]=gene_predictions

#primary
primary_predictions={}
gene='IL2RA'
gene_predictions={}
data=pd.read_csv(f"{data_directory}primary/6_mer_{gene}_PETRA_and_predictions_primary.csv")
data.drop(columns=['Unnamed: 0'], inplace=True)
predictions = [prediction for prediction in data.columns if "filtered_score" not in prediction and "sequence" not
               in prediction]
for column in predictions:
    stats = scipy.stats.spearmanr(data[f"{gene}_filtered_score"], data[column])
    modified_column_name=modify_column_name(column, gene, 'primary')
    gene_predictions[modified_column_name]=stats
primary_predictions[f"{gene}\nprimary"]=gene_predictions

#merge primary and jurkat data to create a heatmap dataframe
...
rho_df_jurkat, pval_df_jurkat = predictions_to_dfs(jurkat_predictions, predictor_order)
rho_df_primary, pval_df_primary = predictions_to_dfs(primary_predictions, predictor_order)

heatmap_R_df=rho_df_jurkat.join(rho_df_primary)
heatmap_p_df=pval_df_jurkat.join(pval_df_primary)
