...
for gene in genes:
    ...
    #select only the variants included in validation screen
    validation_data=pd.read_csv(f"{cwd}/data/6_mer_validation/{gene}_filtered_scores.csv")
    validation_variants=validation_data[['ID']]
    validation_variants.rename(columns={'ID':'sequence'}, inplace=True)
    data=pd.merge(data, validation_variants, on='sequence', how='right')
    data.dropna(subset=[f"{gene}_filtered_score"], inplace=True)
