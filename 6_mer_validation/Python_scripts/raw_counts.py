...
variant_information_file=sys.argv[4]
experiment_folder=sys.argv[5]
...
def generate_GREP_variants_pre_defined(sequence_list, pre_extension, post_extension):
    inserts=sequence_list
    inserts_in_context={}
    for insert in inserts:
        insert_with_context=pre_extension+insert+post_extension
        inserts_in_context[insert]=insert_with_context
    inserts_in_context['WT']=pre_extension+post_extension
    return inserts_in_context
def compute_reverse_complement(sequences):
    reverse_complements = [str(Seq(seq).reverse_complement()) for seq in sequences]
...
pre_sequence, post_sequence=obtain_genomic_context(f"{loci_context_file}")

#obtain list of variants to search for
peg_information=pd.read_csv(f"{variant_information_file}")
peg_information=peg_information[peg_information['gene']==gene]
path_to_peg_file=peg_information['location'].values[0]
orientation_of_peg=peg_information['peg_target_strand'].values[0]

#create variats to GREP
sequences=pd.read_csv(path_to_peg_file)
sequences=sequences['sequence'].tolist()
if orientation_of_peg=='coding':
    inserts=sequences
elif orientation_of_peg=='non-coding':
    inserts=compute_reverse_complement(sequences)
variants_for_detection=generate_GREP_variants_pre_defined(inserts, pre_sequence, post_sequence)
